...
SELECT * FROM EMPLOYEE; -- EMPLOYEE 전부 조회
-- '*' : ALL, 모든, 전부
...
-- <컬럼 값 산술 연산>
-- 컬럼 값 : 테이블 내 한칸 (== 한 셀_)에 작성된 값(DATA)
...
-- 주의사항 1) DISTINCT 구문 SELECT 마다 딱 한 번씩만 작성 가능
...
-- EMPLOYEE 테이블에서 EMAIL의 _앞 글자가 세글자인 사원만 조회
-- 이름, 이메일 조회
 ...
 * EMPLOYEE 테이블에서
 * 이메일 '_' 앞이 4글자이면서
 * 부서코드가 'D9' 또는 'D6'이고 -> AND가 OR보다 우선순위가 높다 () 사용
